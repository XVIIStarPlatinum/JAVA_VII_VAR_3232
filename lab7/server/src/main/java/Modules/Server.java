...
public class Server {
        ...
        try {
            ...
            while (true) {
                ...
                while (keys.hasNext()) {
                    ...
                    logger.info("Обработка ключа началась, объявляю гойду.");
                    try {
                        if (key.isValid()) {
                        ...
                        e.printStackTrace();
                        logger.log(Level.SEVERE, "Поток обработки запроса или отправки запроса был прерван.");
                    }
                    keys.remove();
                }
            }
    ...
    private void processRequest() {
        ...
        if (request.getCommandName() == null) {
            ...
            var commandObjArg = (MusicBandRaw) request.getCommandObjArg();
            var user = request.getUser();
    ...
    private ConsoleApp createConsoleApp() {
        ...
        return new ConsoleApp(
                ...
                new RemoveByIndexCommand(commandHandler),
                new ShuffleCommand(commandHandler),
                new HistoryCommand(commandHandler),
                new PrintFieldDescendingEstablishmentDateCommand(commandHandler),
                new GroupCountingByEstablishmentDateCommand(commandHandler),
